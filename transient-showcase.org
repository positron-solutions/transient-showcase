...
There are several ways to create state.  The [[*Nesting & Flow Control][flow control]] examples in the previous section mainly covered how to get from one command to the other.  This section covers how to save values and then read them later.
...
Transient also has special support for persisting infix values across invocations.  Their interfaces and behavior is a good fit for CLI integrations.  *Coupled with [[*Custom Infix Types][custom infix types]], you can create some seriously rich user expression.*
