   ...
   #+begin_src elisp :tangle yes :var _=wave-prelude                  
     ...
     (transient-define-prefix ts-layout-spaced-out ()
       ...
       ["" ; cannot add another empty string because it will mix suffixes with groups
        ...
        ["\n"] ; vector with just newline will pad
...
*** Readers
   ...
   [[id:0c3f2271-98e0-4db4-b95b-e31975bcbb5e][Advanced/Custom Infix Types]] for
   an example of writing a custom reader that validates input and assigning
   that reader via the class method instead of the =:reader= slot.
...
*** TODO Choices from a function
    ...
    *This section is incomplete.  While it runs, it's likely not demonstrating a
    ...
    Also the history is likely used in completions if done correctly.
