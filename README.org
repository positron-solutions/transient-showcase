...
** Running Examples in Org Mode
  ...
  *Note* If the example above is hard to read, review some [[info:elisp#Top][elisp]] [[*Essential Elisp][syntax]]
    ...
    #+begin_src elisp :tangle yes :var _=simple-parent-child __=wave-prelude
      ...
      (transient-define-prefix ts-parent-with-setup-suffix ()
        ["Simple Parent"
         ...
         ("bc" "become child" ts-suffix-setup-child :transient transient--do-recurse)])
...
**** TODO Errata
     ...
     Also check that the comment in =ts-suffix-setup-child= is fully accurate.
...
*** Default Values
   ...
   #+begin_src elisp
     (transient-define-prefix ts-wave ()
       ...
       ["Arguments"
        ...
        ("-a" "argument" "--argument=")
        ("t" "toggle" "--toggle")
        ;; If you actually need to enforce values, set the :reader slot with
        ;; `transient-read-number-N+' etc
   ...
   *Note*, after setting or saving a value on this transient using the =C-x=
   menu, the next time the transient is set up, it will have a different value.
...
*** Obtaining Missing Scope   
    ...
    Therefore, a method called =transient-init-scope= can be overriden and used
...
** Custom Infix Types
   ...
   *Essential behaviors for your custom infix:*
   ...
   #+name: custom-infixes
   #+begin_src elisp :tangle yes :var _=levels-prelude :var __=print-args-prelude
     ...
     (defun ts--layout-child-desc (layout-child)
       ...
       (message "Layout child for : " (type-of layout-child))
       (let ((description 
              (cond
         ...
         (cl-block nil ; allows cl-return
           (while t
             ...
             (let* ((command (oref transient--prefix command))
                    (loc (read (read-from-minibuffer prompt initial-input nil nil history)))
                    (child (ignore-errors (transient-get-suffix command loc))))
               (when child
                 (cl-return loc)) ; breaks loop
               (message
                (propertize
     ...
     (cl-defmethod transient-init-value ((obj ts-child-infix))
       ...
       (let* ((prefix-value (oref transient--prefix value))
             ...
             (value (car (alist-get key prefix-value)))
             (value-object (transient-get-suffix (oref transient--prefix command) value)))
         (message " --- transient-init-value --- ")
         (message " prefix-value: %s" prefix-value)
         (message " key: %s" key)
         (message " value: %s" value)
         (message " value-object: %s" value-object))

       (if-let* ((prefix-value (oref transient--prefix value))
                 (key (oref obj command))
                 (value (car (alist-get key prefix-value)))
                 (value-object (transient-get-suffix (oref transient--prefix command) value)))
           (progn
             (message "SUCCESFULLY INITIALIZED")             
             (oset obj value value)
             (oset obj value-object value-object))
         (progn
           (message "FAILED INITIALIZATION")
           (oset obj value nil)
           (oset obj value-object nil))))

     (cl-defmethod transient-infix-set ((obj ts-child-infix) value)
       "When the `value' is updated, update the `value-object' as well."

       (message "--- transient-infix-set --- value arg: %s" value)

       (if-let* ((command (oref transient--prefix command))
                 (child (ignore-errors (transient-get-suffix command value))))
           (progn (oset obj value-object child)
                  (oset obj value value))
         (progn
           (message "Invalid value was saved in infix %s" (oref obj command))
           (oset obj value-object nil)
           (oset obj value nil)))
       (message "new value: %s" value)
       (message "value-object: %s" (oref obj value-object))
       (message "value %s" (oref obj value)))

     ;; If you are making a suffix that stores a value, you need to define
     ;; this method.  You also need this method if your value needs some
     ;; processing or use of an alternate value for later rehydration.  Tell
     ;; the prefix what to store when setting / saving
     (cl-defmethod transient-infix-value ((obj ts-child-infix))
       "Return our actual value for rehydration later."

       ;; this is identical to the method defined for `transient-infix', but
       ;; don't forget this if you want history on a suffix for example.
     ...
     Show either the child's description or a default if no child is selected."
       (message "$ obs value-object: " (oref obj value-object))
       (message "$ obs value:" (oref obj value)) 
       ...
       (if-let ((value
                 (if (slot-boundp obj 'value)
                   ...
                   nil))
                (value-object
                 (if (slot-boundp obj 'value-object)
    ...
    representations are bing stored.

*** Reading custom infix values

    =(transient-arg-value "somekey" (transient-args transient-current-command))=

    This works okay if values were exported.

    If you can identify the specific suffix you need, you can retreive the value
    from =(transient-current-suffixes)=, which is also set during
    =(transient--export)=.  This is recommended for custom infixes since
    =transient-arg-value= is somewhat intended for CLI's.

    =transient-get-suffix= is the best because you can use commands or keys in a
    prefix as essentially full addresses.
