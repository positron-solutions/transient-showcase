...
#+INCLUDE: "./package.org::#Package Header"
...
** Overriding slots in the prefix definition
   ...
   #+begin_src elisp :tangle yes :var _=ts-wave-suffix-def
     ...
     (transient-define-prefix ts-wave-overridden ()
       "Wave with overridden suffix behavior"
...
*** TODO Errata
    ...
    *Note*, the uuid is generated on every key input.  Layout updates are fun.
    It should also work when changing descriptions in the layout via hackery.
    凸( ` ﾛ ´ )凸
...
** Single versus multiple commands
   ...
   #+begin_src elisp :tangle yes :var _=wave-prelude __=ts-wave-overridden
...
**** Nesting with multiple commands
    ...
    #+begin_src elisp :tangle yes :var _=wave-prelude __=simple-parent-child
      ...
      (transient-define-prefix ts-simple-parent-with-return ()
        ["Parent With Return"
         ("w" "wave parentally" ts-suffix-wave)
         ("r" "become child with return" ts-simple-child :transient t)])
...
*** Early completion
    ...
    #+begin_src elisp :tangle true
      ...
      (transient-define-prefix ts-complex-messager ()
        "Complex messager, unless `ts--complex' is nil."
      ...
      (transient-define-prefix ts-simple-messager ()
        "Sends messages, optionally switching to complex messages!"
        [["Send Message"
...
** The Magic of Transient    
   ...
   commands and ad-hoc manage their own state.
...
**** TODO The second option requires filtering
...
#+INCLUDE: "./package.org::#Package Footer"    
